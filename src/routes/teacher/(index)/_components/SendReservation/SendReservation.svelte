<script lang="ts">
  ...
  import { sendReservationSchema, type SendReservationSchema } from './schema';
  import Button from '$lib/components/ui/button/button.svelte';
  import DatePicker from '$lib/components/gen/DatePicker.svelte';

  interface Props {
    sendReservationForm: SuperValidated<Infer<SendReservationSchema>>;
  }

  const { sendReservationForm }: Props = $props();

  let open = $state(false);

  const form = superForm(sendReservationForm, {
    validators: zodClient(sendReservationSchema),
...
<Button onclick={() => (open = true)}>Send Reservation</Button>
  ...
  <AlertDialog.Content class="p-0">
    <ScrollArea class="max-h-screen md:max-h-[80dvh]">
      ...
      <AlertDialog.Header class="sticky top-0 z-20 rounded-t-lg p-5 backdrop-blur-lg">
        <AlertDialog.Title>Send Reservation</AlertDialog.Title>
